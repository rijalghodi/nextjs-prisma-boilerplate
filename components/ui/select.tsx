...
const Select = ({
  ...
  // track internal value for uncontrolled usage and to detect hasValue
...
export const selectTriggerVariants = cva(
  `
    ...
    text-foreground data-placeholder:text-muted-foreground focus-visible:border-ring focus-visible:outline-none focus-visible:ring-1 
    cursor-pointer focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 
  ...
  `,
  {
    variants: {
      size: {
        ...
        md: "h-8.5 px-3 text-sm leading-(--text-sm--line-height) gap-1 rounded-full",
...
function SelectTrigger({ className, children, size, ...props }: SelectTriggerProps) {
  const { clearable, value, onValueChange, defaultValue } = React.useContext(SelectContext);
    ...
    >
      ...
      {showClear ? (
        <span
          ...
          className="flex items-center justify-center -me-0.5 rounded-sm hover:bg-muted text-muted-foreground transition-colors cursor-pointer"
          onPointerDown={(e) => {
            e.preventDefault();
            e.stopPropagation();
