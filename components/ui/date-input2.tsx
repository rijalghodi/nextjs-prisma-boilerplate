...
export function DateInput({ value, onChange, readOnly, className, placeholder }: Props) {
  const id = useId();
  ...
  return (
    <Popover>
      <PopoverTrigger>
        <Button className="group/pick-date w-full justify-between" id={id} variant={"outline"}>
          <span
            className={cn("truncate text-muted-foreground", resolvedValue && "text-foreground")}
          >
            {resolvedValue ? formatDateShort(resolvedValue) : placeholder || "Pick a date"}
          </span>
          <CalendarIcon
            aria-hidden="true"
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {eachMonthOfInterval({
                        ...
                        const isCurrent =
                          month.getMonth() === currentMonth && selectedYear === currentYear;
                        ...
                        return (
                          ...
                          >
                            {format(month, "MMM", { locale: idLocale })}
                          </Button>
                        );
                      })}
                    </div>
